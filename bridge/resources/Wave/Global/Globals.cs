        ...
        private void OnSecondSpent(object unused)
        {
            ...
            foreach (Client player in NAPI.Pools.GetAllPlayers())
            {
                if (player.HasSharedData(EntityData.PLAYER_PLAYING))
                {
                    ...
                    {
                        if (player.GetData(EntityData.PLAYER_PLAYED) >= 20)
                        {
                            ...
                            int previousXP = player.GetData(EntityData.PLAYER_XP);
                            player.SetData(EntityData.PLAYER_XP, previousXP + 1);
                            int currentLVL = player.GetData(EntityData.PLAYER_LVL);
                            int nextlvl = player.GetData(EntityData.PLAYER_LVL) + 1;
                            int limit = 6 + 2 * (nextlvl - 2);
                            if (player.GetData(EntityData.PLAYER_XP) == limit)
                            {
                            ...
                            int id = player.GetData(EntityData.PLAYER_SQL_ID);
                            int xp = previousXP + 1;
                            int lvl = player.GetData(EntityData.PLAYER_LVL);
        ...
        private void OnMinuteSpent(object unused)
        {
            ...
            foreach (Client player in NAPI.Pools.GetAllPlayers())
            {
                if (player.HasSharedData(EntityData.PLAYER_PLAYING))
                {
                    ...
                    player.SetData(EntityData.PLAYER_PLAYED, player.GetData(EntityData.PLAYER_PLAYED) + 1);
                    ...
                    CharacterModel character = new CharacterModel
                    {
                        ...
                        played = player.GetData(EntityData.PLAYER_PLAYED),
                        hunger = hunger,
                        thirst = thirst
                    };
