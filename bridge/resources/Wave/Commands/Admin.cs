...
namespace Wave.Commands
{
    class Admin : Script
    {
        ...
        {
            if (player.GetData(EntityData.PLAYER_ADMIN_RANK) < 5) return;
            ...
            Database.Database.SetAdminRank(toAdmin.GetData(EntityData.PLAYER_ADMIN_RANK), adminValue);
            foreach (Client client in NAPI.Pools.GetAllPlayers())
            {
                if (client.GetData(EntityData.PLAYER_ADMIN_RANK) >= 1)
                {
        ...
        {
            if (player.GetData(EntityData.PLAYER_ADMIN_RANK) < 2) return;
        ...
        {
            if (player.GetData(EntityData.PLAYER_ADMIN_RANK) == 0) return;
